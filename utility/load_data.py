...
from utility.TWSF import generate_plugin_A

class Data(object):
    def __init__(self, path, batch_size, session_len):
        self.path = path
        ...
        # self.RI = sp.dok_matrix((self.n_items, self.n_items), dtype=np.float32)
        ...
        # with open(R_file) as f_R:
        #     for l in f_R.readlines():
        #         if len(l) == 0: break
        #         l = l.strip('\n')
        #         items = [float(i) for i in l.split(' ')]
        #         self.RI[int(items[0]), int(items[1])] = items[2]
        # print('item relations loaded')
    ...
    def create_adj_mat(self):
        ...
        #     # RI[int(self.n_items*i/5.0):int(self.n_items*(i+1.0)/5)]
        # adj_mat = adj_mat.todok()
        adj_mat = generate_plugin_A(self.session_len, self.path, n_users=self.n_users, n_items=self.n_items)
